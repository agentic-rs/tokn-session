...
use crate::pi::event::{
    PiAssistantContentBlock, PiEvent, PiMessage, PiMessageEvent, PiToolResultContentBlock,
    PiUserContent, PiUserContentBlock,
};
...
fn normalize_assistant_message(
    ...
    for block in message.content {
        match block {
            ...
            PiAssistantContentBlock::ToolCall {
                id,
                name,
                arguments,
...
fn normalize_tool_result_message(
    ...
    let timestamp = meta
        ...
        .clone()
        .or_else(|| message.timestamp.map(|value| value.to_string()));
    let output = message.details.unwrap_or_else(|| {
        Value::Array(
            message
                .content
                .into_iter()
                .map(tool_result_content_to_value)
                .collect(),
        )
    });
