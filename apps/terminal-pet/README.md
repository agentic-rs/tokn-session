...
Each row includes a state glyph, compact provider badge, session identity,
latest activity, and age, so the display remains useful without color. The
glyph carries the row state; the focus panel retains the full state wording
and explanation. Project identity prefers the Codex Desktop `project_name`,
then `folder_name`, `repository_name`, and the legacy inferred Relay name.
