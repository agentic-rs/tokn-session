...
use service::{IndexRefresh, SessionIndexWake, ViewerService};
use tauri::{Emitter, Manager};
use watcher::{SessionFileWatcher, WatchRequest};

use crate::model::{IndexWorkerError, ViewerProvider};
    ...
    .setup(|app| {
      ...
      tauri::async_runtime::spawn(async move {
        ...
        loop {
          ...
          let result = tauri::async_runtime::spawn_blocking(move || match work {
...
mod tests {
  ...
  use super::{
    IndexRefresh, SessionIndexWake, full_catalog_needs_prompt_retry, index_wake_from_watch_request,
    merge_session_index_wake, session_index_needs_retry, session_index_path_for_home,
