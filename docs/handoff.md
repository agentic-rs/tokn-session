...
silently consume existing records to seed provider normalizer state before
following from EOF. Pass `--replay` to emit existing complete records too.
`--codex-dir`, `--pi-dir`, and `--poll-interval` are shared options.
...
controls provider roots, initial replay, and the periodic rescan interval.
